...
1.  **observations:** Code to get observations from an API request with less than 10K results.

2.  **species-counts:** Code to get a table of species counts from one URL request.

3.  **images:** Code to download iNaturalist images from URL path. In the example, observations are gathered from the iNaturalist API and images are saved with the naming scheme of observation id_photo number so that users can later link the photo to the full observation details.
